...
复盘后确认，这不是一个单点故障，而是可能连续出现的十三层故障链：
...
11. Windows PowerShell 写出的 UTF-8 BOM 使 Native Host presence JSON 报 expected value at line 1 column 1
12. WindowsApps EFS 文件复制返回 UNKNOWN errno=-4094，bundled marketplace 无法刷新
13. Desktop 实际查找内容哈希 runtime，而本地只有 manifest tag 目录，Computer Use helper/transport 因此缺失
```
...
最终成功标志应分开验证。当前版本不能只检查 `nodeRepl.nativePipe` 直接属性；即使该属性为 `undefined`，官方 wrapper transport 仍可能可用：
...
│   └── 09-2026-07-20-完整修复时间线与文件映射.md
├── prompts/
│   └── 复发后给Codex的修复提示词.md
├── scripts/
│   ├── check-codex-runtime.ps1
│   ├── copy-plain-file-template.ps1
│   ├── prewarm-codex-windowsapps-runtime.ps1
│   ├── repair-chrome-app-server-registry.ps1
│   ├── repair-codex-bundled-marketplace-efs.ps1
│   ├── repair-codex-runtime-skew.ps1
│   └── repair-official-node-repl-runtime.ps1
└── logs-examples/
    └── README.md
```
...
11. 若日志含 `UNKNOWN errno=-4094`、`missingHelperPath` 或 `missingTransportModulePath`，看 `docs/09-2026-07-20-完整修复时间线与文件映射.md`，先运行内容哈希/EFS 只读检查。
...
本次最终动态验收和完整文件映射见 [docs/09-2026-07-20-完整修复时间线与文件映射.md](docs/09-2026-07-20-完整修复时间线与文件映射.md)。
