...
- 当前 MSIX 内 bundled 插件版本
- 活动 openai-bundled marketplace 版本
- 已安装插件缓存版本
...
3. 比较当前 MSIX、活动 bundled marketplace 和插件缓存版本。
4. 不要添加普通外部 `node_repl` MCP；它可能让工具可见，但缺少官方可信浏览器/审批上下文。
5. 如果版本不同，先 dry-run 仓库修复脚本；完全退出 Codex、Chrome、Edge 后再应用。
6. 说明当前 turn 不会热加载，需要重启 Codex并新建任务复测。
...
如果日志出现 `Browser security unavailable outside node repl`：

1. 判断普通 stdio MCP 已造成假成功，不是 Chrome 扩展授权问题。
2. 检查 `codex mcp get node_repl`；若存在旧外部 server，计划移除。
3. 对比当前 MSIX、活动 marketplace、插件缓存和 Native Host 的 Chrome / Computer Use 版本。
4. 从当前 MSIX 刷新 bundled marketplace，重装同版本官方插件，再更新 Native Host。
5. 不得修改浏览器安全实现绕过校验。
